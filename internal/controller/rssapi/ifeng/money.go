...
func (ctl *Controller) GetMoney(req *ghttp.Request) {
	var ctx context.Context = context.Background()
	routeArray := strings.Split(req.Router.Uri, "/")
	linkType := routeArray[len(routeArray)-1]
	linkConfig := getMoneyInfoLinks()[linkType]

	cacheKey := "IFENG_MONEY_" + linkConfig.ChannelId
	if value, err := service.GetRedis().Do(ctx,"GET", cacheKey); err == nil {
		if value.String() != "" {
			req.Response.WriteXmlExit(value.String())
		}
	}
	apiUrl := "https://finance.ifeng.com/money/"
	rssData := dao.RSSFeed{
		Title:       "凤凰网|理财 - " + linkConfig.Title,
		Link:        apiUrl,
		Tag:         []string{"理财"},
	...
	if resp := service.GetContent(ctx,apiUrl); resp != ""{
		rssData.Items = moneycommonParser(ctx, resp, linkConfig.ChannelId)
	}
