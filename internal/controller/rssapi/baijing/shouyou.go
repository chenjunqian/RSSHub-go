...
func (ctl *controller) GetShouyou(req *ghttp.Request) {
	...
	if value, err := service.GetRedis().Do(ctx,"GET", "BAIJING_SHOUYOU"); err == nil {
		if value.String() != "" {
	...
	service.GetRedis().Do(ctx,"SET", "BAIJING_SHOUYOU", rssStr)
	service.GetRedis().Do(ctx,"EXPIRE", "BAIJING_SHOUYOU", 60*60*4)
