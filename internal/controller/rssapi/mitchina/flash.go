...
func (ctl *Controller) GetFlash(req *ghttp.Request) {
	...
	if value, err := service.GetRedis().Do(ctx,"GET", "MIT_CHINA_FLASH"); err == nil {
		if value.String() != "" {
	...
	service.GetRedis().Do(ctx,"SET", "MIT_CHINA_FLASH", rssStr)
	service.GetRedis().Do(ctx,"EXPIRE", "MIT_CHINA_FLASH", 60*60*4)
