...
func (ctl *Controller) GetAnswers(req *ghttp.Request) {
	...
	if resp, err := service.GetHttpClient().SetHeaderMap(headers).Get(ctx, answerGetUrl); err == nil {
		...
		if peopleName == "知乎用户" {
			...
			if activitiesResp, err := service.GetHttpClient().SetHeaderMap(headers).Get(ctx, activitiesUrl); err == nil {
				defer func(resp *gclient.Response) {
					err := resp.Close()
					if err != nil {
						g.Log().Error(ctx, err)
		...
		for index := range respDataList {
			...
			if answerResp, err := service.GetHttpClient().SetHeaderMap(headers).Get(ctx, answerDetailUrl); err == nil {
				defer func(resp *gclient.Response) {
					err := resp.Close()
					if err != nil {
						g.Log().Error(ctx, err)
