...
func (ctl *Controller) GetWeeklyRecommend(req *ghttp.Request) {
	...
	if value, err := component.GetRedis().Do(ctx,"GET", "BILIBILI_WEEKLY"); err == nil {
		if value.String() != "" {
	...
	if statusResp := component.GetContent(ctx,apiUrl); statusResp != "" {
		...
		if latestResp := component.GetContent(ctx,latestApiUrl); latestResp != "" {
	...
	component.GetRedis().Do(ctx,"SET", "BILIBILI_WEEKLY", rssStr)
	component.GetRedis().Do(ctx,"EXPIRE", "BILIBILI_WEEKLY", 60*60*3)
