...
func (ctl *Controller) GetAuto(req *ghttp.Request) {
	routeArray := strings.Split(req.Router.Uri, "/")
	linkType := routeArray[len(routeArray)-1]
	linkConfig := getAutoInfoLinks()[linkType]

	cacheKey := "IFENG_AUTO_" + linkConfig.ChannelId
	if value, err := g.Redis().DoVar("GET", cacheKey); err == nil {
		if value.String() != "" {
			_ = req.Response.WriteXmlExit(value.String())
		}
	}
	apiUrl := "https://auto.ifeng.com/" + linkConfig.ChannelId + "/"
	rssData := dao.RSSFeed{
		Title:       "凤凰网|汽车 - " + linkConfig.Title,
		Link:        apiUrl,
		Tag:         []string{"汽车"},
