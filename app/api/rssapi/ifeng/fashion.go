...
func (ctl *Controller) GetFashion(req *ghttp.Request) {
	routeArray := strings.Split(req.Router.Uri, "/")
	linkType := routeArray[len(routeArray)-1]
	linkConfig := getFashionInfoLinks()[linkType]

	cacheKey := "IFENG_FASHION_" + linkConfig.ChannelId
	if value, err := g.Redis().DoVar("GET", cacheKey); err == nil {
		if value.String() != "" {
			_ = req.Response.WriteXmlExit(value.String())
		}
	}
	apiUrl := "https://fashion.ifeng.com/" + linkConfig.ChannelId + "/"
	rssData := dao.RSSFeed{
		Title:       "凤凰网|时尚 - " + linkConfig.Title,
		Link:        apiUrl,
		Tag:         []string{"时尚"},
